...
wpList = {
	...
	ajaxDel: function( element, settings ) {
		...
		if ( 'none' !== settings.delColor ) {
			$eventTarget.css', settings.delColor ).fadeOut( 350, function() {
				list.wpList.recolor();
		...
		settings.success = function( response ) {
			...
			if ( ! parsedResponse || parsedResponse.errors ) {
				$eventTarget.stop().stop().css' ).show().queue( function() {
					list.wpList.recolor();
					$( this ).dequeue();
				} );
	...
	ajaxDim: function( element, settings ) {
		...
		settings.success = function( response ) {
			...
			if ( ! parsedResponse || parsedResponse.errors ) {
				$eventTarget.stop().stop().css']( settings.dimClass ).show().queue( function() {
					list.wpList.recolor();
					$( this ).dequeue();
				} );
	...
	add: function( element, settings ) {
		...
		if ( 'none' !== settings.addColor ) {
			$element.css', settings.addColor ).animate( { backgroundColor: wpList.getColor( $element ) }, {
				complete: function() {
					$( this ).css' );
				}
			} );
