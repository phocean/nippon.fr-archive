...
$.imgAreaSelect = function (img, options) {
    ...
    function adjust() {
        ...
        parOfs = /absolute|relative/.test($parent.css')) ?
    ...
    function update(resetKeyPress) {
        ...
        if (msie && $border.outerWidth() - $border.innerWidth() == 2) {
            $border.css', 0);
            setTimeout(function () { $border.css'); }, 0);
        }
    }
    ...
    function setOptions(newOptions) {
        ...
        if (o = options.borderColor1)
            $($border[0]).css', borderColor: o });
        if (o = options.borderColor2)
            $($border[1]).css', borderColor: o });

        /* Append all the selection area elements to the container box */
        $box.append($area.add($border).add($areaOpera)).append($handles);

        if (msie) {
            if (o = ($outer.css').match(/opacity=(\d+)/))
                $outer.css', o[1]/100);
            if (o = ($border.css').match(/opacity=(\d+)/))
                $border.css', o[1]/100);
        }
     ...
     * .fadeIn().
     */
    $box.add($outer).css', position: position,
        overflow: 'hidden', zIndex: zIndex || '0' });
    $box.css({ zIndex: zIndex + 2 || 2 });
    $area.add($border).css', fontSize: 0 });
